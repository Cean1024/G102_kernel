/*
 * sound\soc\sunxi\sunxi_sndddsp1.c
...
static int sunxi_sndddsp1_hw_params(struct snd_pcm_substream *substream,
...
static struct snd_soc_ops sunxi_snddaudio_ops = {
	.hw_params 		= sunxi_sndddsp1_hw_params,
};
...
static int  sunxi_sndddsp1_dev_probe(struct platform_device *pdev)
{
	int ret = 0,i;
	...
	for(i=0;i<(card->num_links);i++){
		sunxi_snddaudio_dai_link[i].cpu_dai_name = NULL;
		sunxi_snddaudio_dai_link[i].cpu_of_node = of_parse_phandle(np,
					"sunxi,daudio1-controller", 0);
		if (!sunxi_snddaudio_dai_link[i].cpu_of_node) {
			dev_err(&pdev->dev,
...
static int  __exit sunxi_sndddsp1_dev_remove(struct platform_device *pdev)
{
...
static struct platform_driver sunxi_daudio_driver = {
	...
	.probe = sunxi_sndddsp1_dev_probe,
	.remove = __exit_p(sunxi_sndddsp1_dev_remove),
};
...
MODULE_AUTHOR("kean");
MODULE_DESCRIPTION("SUNXI_sndddsp ALSA SoC audio driver");
MODULE_LICENSE("GPL");
