...
struct snd_soc_codec *ziven_codec;
static u32 power_sw;
...
static void set_dsp_mute(bool val){
	//static unsigned int mute;
	
	/* mute during register sync */
	if(val){
		//mute = snd_soc_read(ziven_codec, STA32X_MVOL);
		printk("++++read mute on+++++++++++\n");
		snd_soc_write(ziven_codec, STA32X_MMUTE, 0x16);
	}
	else{
		printk("======mute off ===========\n");
		snd_soc_write(ziven_codec, STA32X_MMUTE, 0x10);
	}
}
EXPORT_SYMBOL_GPL(set_dsp_mute);
...
static int sta32x_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, STA32X_CONFF, 0xd8);
	snd_soc_write(codec, STA32X_CONFD, 0x50);
	snd_soc_write(codec, STA32X_MVOL, 0xff);
	snd_soc_write(codec, STA32X_MMUTE, 0x10);
	snd_soc_write(codec, STA32X_AUTO1, 0x80);
	
	snd_soc_write(codec, STA32X_AUTO2, 0x00);
			    ...
			    const struct i2c_device_id *id)
{
	...
	power_sw = of_get_named_gpio_flags(np, "power-sw", 0, (enum of_gpio_flags *)&config_sw);
	if (!gpio_is_valid(power_sw)) {
		pr_err("failed to get power_gpio gpio from dts,power_gpio:%d\n", power_sw);
	} else {
		ret = gpio_request(power_sw, "power_sw");
		if (ret) {
			pr_err("failed to request power_gpio  gpio\n");
		} else {
		printk("power_sw = %d\n", power_sw);
			gpio_direction_output(power_sw, 1);
			gpio_set_value(power_sw, 1);
		}
	}
